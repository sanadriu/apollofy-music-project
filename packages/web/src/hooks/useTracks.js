...
export function usePrefetchTracks(nextPage, currentGenre = undefined, currentLimit = 10, sort = undefined, order = 'desc', userId = undefined) {
  const queryClient = useQueryClient();
  queryClient.prefetchQuery(
    [queryKeys.tracks, nextPage, currentGenre],
    () => tracksApi.getTracks(nextPage, currentGenre, currentLimit, sort, order, userId)
  )
}
